...
if (defined('MODULE_MITS_EMBEDDED_VIDEOS_STATUS') && MODULE_MITS_EMBEDDED_VIDEOS_STATUS == 'true') {
  ...
  function mits_get_products_videos($products_id = '') {
    $videos_query = "SELECT * FROM " . TABLE_MITS_EMBEDDED_VIDEOS . " WHERE products_id = " . (int)$products_id ." ORDER BY video_nr";
    $products_videos_query = xtDBquery($videos_query);
    $results = array();
    while ($row = xtc_db_fetch_array($products_videos_query,true)) {
