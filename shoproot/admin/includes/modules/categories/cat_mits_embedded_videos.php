        ...
        for ($v = 1, $c = $countVideoFields; $v < $c; $v++) {
            ...
            if (isset($check_query) && xtc_db_num_rows($check_query) > 0) {
                if (empty($categories_data['video_url_' . $v . '_' . $language_id]) && empty($categories_data['video_source_id_' . $v . '_' . $language_id])) {
                    ...
                    if (isset($categories_data['embedded_video_id_' . $v . '_' . $language_id]) && !empty($categories_data['embedded_video_id_' . $v . '_' . $language_id])) {
                        xtc_db_perform(
                          TABLE_MITS_EMBEDDED_VIDEOS,
                          $sql_video_data_array,
                          'update',
                          "categories_id = " . $categories_id . " AND embedded_video_id = " . $categories_data['embedded_video_id_' . $v . '_' . $language_id] . " AND languages_id = " . $language_id . " AND video_nr = " . $categories_data['video_nr_' . $v . '_' . $language_id]
                        );
                    } else {
                        xtc_db_perform(
                          TABLE_MITS_EMBEDDED_VIDEOS,
                          $sql_video_data_array,
                          'update',
                          "categories_id = " . $categories_id . " AND languages_id = " . $language_id . " AND video_nr = " . $categories_data['video_nr_' . $v . '_' . $language_id]
                        );
                    }
                }
            } else {
                ...
                if (empty($products_data['video_url_' . $v . '_' . $language_id]) && empty($products_data['video_source_id_' . $v . '_' . $language_id])) {
                    ...
                    if (isset($products_data['embedded_video_id_' . $v . '_' . $language_id]) && !empty($products_data['embedded_video_id_' . $v . '_' . $language_id])) {
                        xtc_db_perform(
                          ...
                          $sql_video_data_array,
                          'update',
                          "products_id = " . $products_id . " AND embedded_video_id = " . $products_data['embedded_video_id_' . $v . '_' . $language_id] . " AND languages_id = " . $language_id . " AND video_nr = " . $products_data['video_nr_' . $v . '_' . $language_id]
                        );
                    } else {
                        xtc_db_perform(
                          TABLE_MITS_EMBEDDED_VIDEOS,
                          $sql_video_data_array,
                          'update',
                          "products_id = " . $products_id . " AND languages_id = " . $language_id . " AND video_nr = " . $products_data['video_nr_' . $v . '_' . $language_id]
                        );
                    }
                }
            } else {
